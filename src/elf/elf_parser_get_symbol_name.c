...
char *elf_parser_get_symbol_name(char *elf_filepath, uint64_t addr)
{
    ...
    if (!function_name) {
        ...
        //function_name = elf_parser_find_name(addr - (16 * get_nb_plt_sym(elf, sh_link_dyntab, data)), &section_header, section, elf);
        function_name = elf_parser_find_name(addr - (16 * get_nb_plt_sym(elf)), &section_header, section, elf);
        if (!function_name)
            function_name = elf_parser_find_name(addr, &section_header, section, elf);
    }
    elf_end(elf);
